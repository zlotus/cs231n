...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dloss_dout1, dW2, db2 = affine_backward(dloss_dscores, cache2)
    _, dW1, db1 = affine_relu_backward(dloss_dout1, cache1)
    
    loss += 0.5*reg*np.sum(W1*W1) + 0.5*reg*np.sum(W2*W2)       # regularization
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(self.num_layers):
      if i == 0:
        ins, outs = input_dim, hidden_dims[i]
      elif i == self.num_layers-1:
        ins, outs = hidden_dims[i-1], num_classes
      else:
